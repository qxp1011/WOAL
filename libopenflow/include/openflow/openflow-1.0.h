...
#define OFPP_FIRST_RESV OFP_PORT_C(0xfffffff8) /* First assigned reserved port. */
#define OFPP_LAST_RESV  OFP_PORT_C(0xffffffff) /* Last assigned reserved port. */
...
struct ofp10_phy_port {
    ...
    ACE_UINT32 config;        /* Bitmap of OFPPC_* and OFPPC10_* flags. */
    ACE_UINT32 state;         /* Bitmap of OFPPS_* and OFPPS10_* flags. */
...
struct ofp10_port_mod {
    ...
    ACE_UINT32 advertise;     /* Bitmap of "ofp_port_features"s.  Zero all
...
struct ofp10_queue_get_config_request {
    ACE_UINT16 port;          /* Port to be queried. Should refer
...
struct ofp10_packet_in {
    ...
    ACE_CDR::Octet pad;
    ACE_CDR::Octet data[0];        /* Ethernet frame, halfway through 32-bit word,
...
struct ofp10_action_output {
    ACE_UINT16 type;                  /* OFPAT10_OUTPUT. */
    ACE_UINT16 len;                   /* Length is 8. */
    ACE_UINT16 port;                  /* Output port. */
...
struct ofp10_action_enqueue {
    ACE_UINT16 type;            /* OFPAT10_ENQUEUE. */
    ACE_UINT16 len;             /* Len is 16. */
    ACE_UINT16 port;            /* Port that queue belongs. Should
                                 refer to a valid physical port
                                 (i.e. < OFPP_MAX) or OFPP_IN_PORT. */
    ACE_CDR::Octet pad[6];           /* Pad for 64-bit alignment. */
    ACE_UINT32 queue_id;        /* Where to enqueue the packets. */
};
...
struct ofp10_packet_out {
    ACE_UINT32 buffer_id;           /* ID assigned by datapath or UINT32_MAX. */
...
struct ofp10_match {
    ...
    ACE_UINT16 dl_vlan;          /* Input VLAN. */
    ACE_CDR::Octet dl_vlan_pcp;       /* Input VLAN priority. */
    ACE_CDR::Octet pad1[1];           /* Align to 64-bits. */
    ACE_UINT16 dl_type;          /* Ethernet frame type. */
    ACE_CDR::Octet nw_tos;            /* IP ToS (DSCP field, 6 bits). */
    ACE_CDR::Octet nw_proto;          /* IP protocol or lower 8 bits of
                                  ARP opcode. */
...
struct ofp10_flow_mod {
    ...
    /* Flow actions. */
    ACE_UINT16 command;             /* One of OFPFC_*. */
    ...
    ACE_UINT16 out_port;            /* For OFPFC_DELETE* commands, require
...
struct ofp10_stats_msg {
    ...
    ACE_UINT16 flags;             /* Requests: always 0.
...
struct ofp10_flow_stats_request {
    ...
    ACE_UINT16 out_port;        /* Require matching entries to include this
...
struct ofp10_flow_stats {
    ...
    ACE_UINT32 duration_nsec;   /* Time flow has been alive in nanoseconds
                                 beyond duration_sec. */
    ...
    wpl_32aligned_be64 cookie;       /* Opaque controller-issued identifier. */
    wpl_32aligned_be64 packet_count; /* Number of packets in flow. */
    wpl_32aligned_be64 byte_count;   /* Number of bytes in flow. */
...
struct ofp10_table_stats {
    ...
    ACE_UINT32 wildcards;      /* Bitmap of OFPFW10_* wildcards that are
                                supported by the table. */
    ACE_UINT32 max_entries;    /* Max number of entries supported. */
    ACE_UINT32 active_count;   /* Number of active entries. */
    wpl_32aligned_be64 lookup_count;  /* # of packets looked up in table. */
    wpl_32aligned_be64 matched_count; /* Number of packets that hit table. */
};
OFP_ASSERT(sizeof(struct ofp10_table_stats) == 64);

/* Stats request of type OFPST_PORT. */
struct ofp10_port_stats_request {
    ACE_UINT16 port_no;        /* OFPST_PORT message may request statistics
...
struct ofp10_port_stats {
    ...
    wpl_32aligned_be64 rx_packets;     /* Number of received packets. */
    wpl_32aligned_be64 tx_packets;     /* Number of transmitted packets. */
    wpl_32aligned_be64 rx_bytes;       /* Number of received bytes. */
    wpl_32aligned_be64 tx_bytes;       /* Number of transmitted bytes. */
    wpl_32aligned_be64 rx_dropped;     /* Number of packets dropped by RX. */
    wpl_32aligned_be64 tx_dropped;     /* Number of packets dropped by TX. */
    wpl_32aligned_be64 rx_errors; /* Number of receive errors.  This is a
                                     super-set of receive errors and should be
                                     great than or equal to the sum of all
                                     rx_*_err values. */
    wpl_32aligned_be64 tx_errors; /* Number of transmit errors.  This is a
                                     super-set of transmit errors. */
    wpl_32aligned_be64 rx_frame_err; /* Number of frame alignment errors. */
    wpl_32aligned_be64 rx_over_err;  /* Number of packets with RX overrun. */
    wpl_32aligned_be64 rx_crc_err;   /* Number of CRC errors. */
    wpl_32aligned_be64 collisions;   /* Number of collisions. */
};
...
struct ofp10_queue_stats_request {
    ACE_UINT16 port_no;        /* All ports if OFPP_ALL. */
...
struct ofp10_queue_stats {
    ...
    wpl_32aligned_be64 tx_bytes;   /* Number of transmitted bytes. */
    wpl_32aligned_be64 tx_packets; /* Number of transmitted packets. */
    wpl_32aligned_be64 tx_errors;  /* # of packets dropped due to overrun. */
};
