...
void 
ACE_Message_Block::append (ACE_Message_Block *block)
{
  ACE_Message_Block *p = this;
  while (p)
  {
    if (p->cont())
    {
...
std::string
ACE_Message_Block::flatten_chained()
{
  std::string strRet;
  strRet.reserve(total_length() + 1);

  for (ACE_Message_Block *i = this; NULL != i; i = i->cont_)
    {
	  strRet.append(i->rd_ptr(), i->length());
    }
