...
ACE_CDR::Boolean 
ACE_InputCDR::pre_read (void *x, 
                        ACE_CDR::ULong length)
{
  if ((0 == x) || (0 == length))
    return (good_bit_ = false);
  
  const ACE_Message_Block *s = start ();
  if (0 == s)
    return (good_bit_ = false);

  ACE_Data_Block *db = s->data_block ();
  if (0 == db)
    return (good_bit_ = false);
  ...
  ACE_OS::memcpy(x, base, length);
  return (good_bit_ = true);
}
