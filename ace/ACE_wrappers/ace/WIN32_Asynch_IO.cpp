                                    ...
                                    int signal_number)
{
  ...
  // Shared write, fix "not socket handle " problem, 2013/12/12
  int return_val = this->shared_write_x(result);
...
//yijian add for fix win32 write error="not socket handle" problem

int
ACE_WIN32_Asynch_Write_File::shared_write_x (ACE_WIN32_Asynch_Write_File_Result *result)
{
  u_long bytes_written;
  if (result->bytes_to_write () > MAXDWORD)
    {
  ...
  ACE_DEBUG ((LM_DEBUG,
            "CTapWin32::shared_write_x,handle=%d, bytes_to_write=%d\n",result->handle (), bytes_to_write));
