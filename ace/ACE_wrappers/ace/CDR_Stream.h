...
public:
   ...
   * Return @c false on failure and @c true on success.
   */
  //@{ @name Preread basic IDL types without advancing the rd_ptr.
  //Note: The following implementation doesnot take alignment into 
  //      consideration.
  //Introduced by Mazhenhua, refered WFACE.
  ACE_CDR::Boolean pre_read (void *x, 
  							 ACE_CDR::ULong length);
  //@}
...
// Mazhenhua add operator>> for ACE_CDR::Octet.
extern ACE_Export ACE_CDR::Boolean operator>> (ACE_InputCDR &is,
                                               ACE_CDR::Octet &x);
